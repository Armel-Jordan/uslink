...
- **Before committing**: `npm run typecheck` **and** `npm run test:rls`. `supabase/functions` is Deno and excluded from `tsconfig.json` on purpose — nothing typechecks it, and nothing typechecks SQL either, which is exactly why the RLS suite exists.
